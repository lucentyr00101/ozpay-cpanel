...
import type { MemberItem, Params } from './data.d';

// mock - generate deposit list
const genList = (current: number, pageSize: number) => {
  const tableListDataSource: MemberItem[] = [];
  ...
  for (let i = 0; i < pageSize; i += 1) {
    ...
    tableListDataSource.push({
      ...
      member: `whiterabbit_${index}`,
      status: Math.random() < 0.5,
      action: 'here',
      createdBy: 'Chuy2022',
      creationTime: (Date.now() - Math.floor(Math.random() * 2000)).toString(),
    });
  }
...
export default {
  'GET /api/member': getMerchantList,
};
