...
import { fetchMemberTRXMerchantExport, fetchMerchantReports } from '../../shared/merchant/service';
...
const MemberMerchantReports: FC = () => {
  const t = useIntl();
  ...
  const downloadFromBlob = (blob: Blob) => {
    ...
    link.download = `${t.formatMessage({ id: 'menu.MemberReports' })}-${t.formatMessage({
      id: 'menu.MemberReports.Merchant',
    })}_${username}.xlsx`;
    link.click();
  };
  ...
  const handleDownloadExcel = async () => {
    ...
    const res = await fetchMemberTRXMerchantExport(filter);
    downloadFromBlob(res);
  };

  // const [paginationProps, setPaginationProps] = useState(defaultPaginationProps);
  ...
  function onChangeMemberMerchant(e: any) {
    const value = e.target.value;
    setRadioValue(value);
    actionRef?.current?.reload();
  }

  const handleFetchMemberMerchantReports = async (values: any, sort: any) => {
    ...
    const response = await fetchMerchantReports(filter);
  ...
  useEffect(() => {
    const currMenuAccess = access?.MemberTRXReports.Merchant;
    const merchanrReport = Object.keys(currMenuAccess).filter((key) => {
      return currMenuAccess[key] === true;
    });
    setPageAccess(merchanrReport.length > 0);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (filterRef) {
      setFilters('member-reports-merchant', filterRef);
    }
  }, [filterRef]);
    ...
    >
      <PageContainer title={false}>
        ...
        <ProTable<MerchantReportItem>
          ...
          request={handleFetchMemberMerchantReports}
          formRef={filterRef}
          onSubmit={(v: any) =>
            cookies.set(filterCookieMap['member-reports-merchant'], v, { path: '/' })
          }
          onReset={() => cookies.remove(filterCookieMap['member-reports-merchant'], { path: '/' })}
          search={
            access?.MemberTRXReports.Merchant['View(Ownonly)'] ||
            access?.MemberTRXReports.Merchant['View(forAll)']
          }
          columns={
            access?.MemberTRXReports.Merchant['View(Ownonly)'] ||
            access?.MemberTRXReports.Merchant['View(forAll)']
          ...
          options={false}
          toolBarRender={() => [
            <Radio.Group onChange={onChangeMemberMerchant} value={radioValue} key="radioButtonGroup">
            ...
            <Access key="add" accessible={access?.MemberTRXReports.Merchant.Export}>
              <Button
                type="primary"
                key="btnExport"
...
export default MemberMerchantReports;
