...
import { fetchMerchantAmounts, fetchMemberWithdrawalStatusPercentage, getMemberMerchantLastSevenDays, memberMerchantFinanceByMerchant } from "./service";
...
const Member: FC = () => {
  ...
  const filteredMemberMerchantLastSevenDays = async ()=> {
    const response = await getMemberMerchantLastSevenDays();
  ...
  const filteredMemberWithdrawalStatusPercentage = async (value: RangePickerValue)=> {
    const filter: any = {
    ...
    const response = await fetchMemberWithdrawalStatusPercentage(filter);
  ...
  const handleWithdrawalStatusPercentageChange = (value: RangePickerValue) => {
    ...
    filteredMemberWithdrawalStatusPercentage(value);
  };

  //Merchant
  const filteredMerchantFinanceByMerchant = async ()=> {
    const response = await memberMerchantFinanceByMerchant();
  ...
  useEffect(()=>{
    if((currentUser && currentUser.userType === USER_TYPE.Admin) || (currentUser && currentUser.userType === USER_TYPE.Non_Admin && currentUser.parentMerchantType === null)){
      filteredMemberMerchantLastSevenDays();
      filteredMemberWithdrawalStatusPercentage([moment(today,'yyyy-MM-DD'), moment(today,'yyyy-MM-DD')]);
    }
    else {
    ...
    return (
      <Access 
      accessible={access?.Dashboard.Member}
      fallback={
        <Result
          status="404"
          style={{
