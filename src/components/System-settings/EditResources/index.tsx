...
const ResourceModal: React.FC<CollectionCreateFormProps> = ({
  ...
  if (resource && Object.keys(resource).length) {
    form.setFieldsValue({
      ...resource,
      name: JSON.parse(resource.name as string)['en-US'],
      chineseName: JSON.parse(resource.name as string)['zh-CN'],
    });
  }
  ...
  return (
    <ModalForm
      ...
      onFinish={(values: any) => {
        const json = {
          'en-US': values?.name,
          'zh-CN': values?.chineseName,
        };
        const resourceCurrent = {
          id: resource.id,
          code: values?.code || resource.code,
          openWay: values?.openWay || resource.openWay,
          name: JSON.stringify(json) || resource.name,
          sort: values?.sort || resource.sort,
          status: values?.status || resource.status,
          type: values?.type || resource.type,
        };

        onUpdate(resourceCurrent);
        return values;
      }}
      visible={visible}
      title={resourceTitle}
